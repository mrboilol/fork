...
if not hg or not hg.ammotypes then
    ErrorNoHalt("[9x19 Rubber] Ошибка: основная система аммо не найдена!\n")
    return
end
...
local function register9rmm()
    ...
    print("[9x19 Rubber] Успешно загружен!")
end

if game.IsDedicated() or game.GetMap() ~= "" then
    register9rmm()
else
    hook.Add("Initialize", "init-ammo-10mm", register9rmm)
end
...
if hg.ammotypesallowed then
    ...
    hg.ammotypesallowed["9x19 Rubber"] = hg.ammotypes["9x19rubber"]
end
...
if hg.ammotypeshuy then
    ...
    hg.ammotypeshuy["9x19 Rubber"] = table.Copy(hg.ammotypes["9x19rubber"])
    hg.ammotypeshuy["9x19 Rubber"].name = "9x19rubber"
end

-- Создаем обратную связь для системы дропа
if SERVER then
    -- Добавляем хук для обработки создания энтити при дропе
    hook.Add("PlayerAmmoDrop", "9rmm_drop_fix", function(ply, ammotype, count)
        -- Если это наш калибр, убеждаемся что создается правильная энтити
        if ammotype == "9x19rubber" then
...
print("[9x19 Rubber] Файл загружен, используйте ключ '9x19 Rubber' в коде винтовки")
