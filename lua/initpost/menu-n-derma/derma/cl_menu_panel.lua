...
function PANEL:Init()
    ...
    local logoVerticalOffset = MenuUnit(8)
    self.logoPanel = logoPanel
    logoPanel:Dock(BOTTOM)
    logoPanel:SetTall(1)
    logoPanel:SetMouseInputEnabled(true)
    logoPanel:DockMargin(0, 0, 0, MenuUnit(menu_title.spacing + menu_title.logo_rise))
    self.logoBaseMargin = MenuUnit(menu_title.spacing + menu_title.logo_rise)
    logoPanel.Think = function(this)
        ...
        this:SetTall(math.ceil(logoVerticalOffset + drawH * (1 + menu_live.title_hover_scale) + splashH + MenuUnit(menu_title.splash_spacing + 7)))
    end
    logoPanel.Paint = function(this, w, h)
        ...
        local drawY = math.Clamp(logoVerticalOffset + driftY + shakeY, 0, maxY)
        surface.SetDrawColor(255, 255, 255, 255)
