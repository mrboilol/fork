...
hook.Add("Think", "Fake", function()
	...
	for i, ply in player.Iterator() do
		...
		if not wep.RagdollFunc then
			...
			local meleeHandControl = wep.ismelee2 and (ply:KeyDown(IN_ATTACK) or ply:KeyDown(IN_ATTACK2) or ply:KeyDown(IN_USE))
			if not fallCoverActive and not manualHoldWound and (!IsValid(ragdoll.ConsLH) and (ply:KeyDown(IN_ATTACK) and !ishgweapon(wep)) or (((ishgweapon(wep) and (!wep:IsResting() or ply:KeyDown(IN_FORWARD) or ply:KeyDown(IN_BACK))) and (ply:KeyDown(IN_USE) or ply:KeyDown(IN_ATTACK2))) or meleeHandControl)) then// || ply:InVehicle() then
				if org.canmove then
					//if !ply:InVehicle() then
						ang2:Set(angles)
			...
			if not fallCoverActive and (!IsValid(ragdoll.ConsRH) and ply:KeyDown(IN_ATTACK2) or (ishgweapon(wep) and ply:KeyDown(IN_USE)) or meleeHandControl) then// || ply:InVehicle() then
				if org.canmove then
			...
			if ply:KeyDown(IN_SPEED) and org.canmove and !org.larmamputated and !org.larmupamputated and (!ply.HandsStun or ply.HandsStun < CurTime()) then
				...
				if (ragdoll.cooldownLH or 0) < time and not IsValid(ragdoll.ConsLH) and not wepinreload then
					...
					if IsValid(choking) or (trace.Hit and not trace.HitSky) then
						...
						local forceLimit = leftArmInjured and 600 or ent:IsWorld() and 10000 or 0
						...
						if IsValid(cons) then
							...
							if leftArmInjured then org.painadd = org.painadd + 2 end
			...
			if ply:KeyDown(IN_WALK) and org.canmove and !(ishgweapon(wep) or wep.ismelee2) and !org.rarmamputated and !org.rarmupamputated and (!ply.HandsStun or ply.HandsStun < CurTime()) then
				...
				if (ragdoll.cooldownRH or 0) < time and not IsValid(ragdoll.ConsRH) then
					...
					if IsValid(choking) or (trace.Hit and not trace.HitSky) then
						...
						local forceLimit = rightArmInjured and 600 or ent:IsWorld() and 10000 or 0
						...
						if IsValid(cons) then
							...
							ragdoll.ConsRH = cons
							cons.grabStarted = time
							if rightArmInjured then org.painadd = org.painadd + 2 end
