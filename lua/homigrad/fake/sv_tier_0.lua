...
function fakeBoneFlop.ResolveBone(limb, segment)
	...
	if segment == nil then
		segment = 2
	elseif isnumber(segment) then
		segment = math.Clamp(math.Round(segment), 1, 3)
	end
...
local function normalizeFloppyState(state)
	...
	if state == true then return {state = "broken"} end

	return {state = "broken"}
end

local function sameFloppyState(a, b)
	if a == b then return true end
	if not istable(a) or not istable(b) then return false end

	return a.state == b.state and a.limb == b.limb and a.segment == b.segment
end
...
function fakeBoneFlop.FlagBone(org, bone, active, state)
	...
	if active then
		...
		if current and sameFloppyState(current, wanted) then return false end
...
local function getRagdollFloppyOwner(rag)
...
function fakeBoneFlop.GetStoredBoneState(org, bone)
	if not org or not org.fake_floppy_bones then return end

	local state = org.fake_floppy_bones[bone]
	if not state then return end

	return normalizeFloppyState(state)
end
...
function fakeBoneFlop.GetRagdollBoneState(rag, bone)
	...
	if rag.hg_floppy_bones and rag.hg_floppy_bones[bone] then
		return normalizeFloppyState(rag.hg_floppy_bones[bone])
	end

	local owner = getRagdollFloppyOwner(rag)
	if IsValid(owner) and owner.organism then
		return fakeBoneFlop.GetStoredBoneState(owner.organism, bone)
	end
end

function fakeBoneFlop.IsBoneFloppy(rag, bone)
	return fakeBoneFlop.GetRagdollBoneState(rag, bone) ~= nil
end

function fakeBoneFlop.GetPhysBoneState(rag, physNumber, alreadyReal)
	if not IsValid(rag) then return end
	...
	local state = fakeBoneFlop.GetRagdollBoneState(rag, boneName)
	if not state then return end

	return state, boneName
end

function fakeBoneFlop.IsPhysFloppy(rag, physNumber, alreadyReal)
	return fakeBoneFlop.GetPhysBoneState(rag, physNumber, alreadyReal) ~= nil
end

function fakeBoneFlop.GetPhysControlMultiplier(rag, physNumber, alreadyReal)
	local state = fakeBoneFlop.GetPhysBoneState(rag, physNumber, alreadyReal)
	if not state then return false, 1 end

	if state.state == "dislocated" then
		return true, 0.18
	end
...
function fakeBoneFlop.BendBone(rag, bone, forceMul)
	...
	if state and state.state == "dislocated" then
		forceMul = forceMul * 1.25
	end
...
function fakeBoneFlop.ApplyBone(rag, bone, state)
	...
	if rag.hg_floppy_bones and rag.hg_floppy_bones[bone] then
		if state then
			rag.hg_floppy_bones[bone] = normalizeFloppyState(state)
		end
		return
	end
	...
	state = state or fakeBoneFlop.GetRagdollBoneState(rag, bone) or {state = "broken"}
