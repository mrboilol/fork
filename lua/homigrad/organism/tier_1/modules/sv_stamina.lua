...
module[2] = function(owner, org, timeValue)
	...
	stamina.sub = stamina.sub * (1 + despair * 0.6) * (1 - goodmood * 0.5)
	...
	stamina.max = (org.superfighter and 2 or 1) * ((stamina.range * (1 - (org.pneumothorax) / 2) + org.adrenaline * 20 ) * math.max(1 - org.hemotransfusionshock,0.2)) * math.max(1 - (org.hungry/100),0.65)
	    ...
	    stamina[1] = max(stamina[1] - stamina.sub * timeValue * 16 * 2.0 * (2 - (org.o2[1] / org.o2.range)), 0)
...
hook.Add("FinishMove", "!homigrad-organism", function(ply, move)
	...
	if vel ~= vecZero then ply.organism.stamina[1] = max(ply.organism.stamina[1] - ply:GetJumpPower() / 10 * 2.0,0) end
