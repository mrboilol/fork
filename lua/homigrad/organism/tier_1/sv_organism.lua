...
local hg_otrubsound = ConVarExists("hg_otrubsound") and GetConVar("hg_otrubsound") or CreateConVar("hg_otrubsound", "4", FCVAR_ARCHIVE + FCVAR_REPLICATED + FCVAR_NOTIFY, "Unconscious (otrub) audio: 0 = unconscious beat, 1 = altotrub, 2 = sleepy, 3 = itssoover, 4 = nga im cooked, 5 = REM dying, 6 = itshopeless, 7 = fuck", 0, 7)
...
function hg.organism.UpdatePerfusion(owner, org, timeValue)
	...
	-- volume. Two litres begins the blackout/death range; delivery is only zero
	-- once the terminal 1 L floor is reached.
	local blood = math.max(org.blood or 0, 0)
	-- Continuous blood-volume delivery: perfusion falls steadily through the
	-- 2.5-2 L danger band and reaches zero only at terminal volume. Do not
	-- multiply this again in the cardiac oxygen calculation.
