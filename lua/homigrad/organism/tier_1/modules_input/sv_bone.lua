...
local function hasNewThoughts(org)
	local owner = org.owner
	return IsValid(owner) and owner:IsPlayer() and owner:GetInfoNum("hg_newthoughts", 0) > 0
end
...
local function sendThought(org, msg, key, delay, clr)
	if hasNewThoughts(org) then
		local owner = org.owner
		if owner.Thought then return owner:Thought(msg, delay or 1, key, 0, clr) end
		return notifyOwner(org, msg, delay or 1, key, 0, nil, clr)
	end
end
...
local function upper_limb(org, bone, dmg, dmgInfo, amputate_key, limb_key, segment, boneindex, dir, hit, ricochet)
	...
	if dmgInfo:IsDamageType(DMG_CRUSH) and dmg > amputateThreshold then
		local targetLimb = org[limb_key.."amputated"] and amputate_key or limb_key
		if !org[targetLimb.."amputated"] then hg.organism.AmputateLimb(org, targetLimb, nil, dmgInfo) end
		return 0
	end
