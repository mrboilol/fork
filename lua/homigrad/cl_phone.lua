...
local function OpenNumberPanel(phone)
	...
	local textNumber = vgui.Create("DButton", panel)
	textNumber:SetPos(12, 222)
	textNumber:SetSize(226, 32)
	textNumber:SetText("TEXT THIS NUMBER")
	textNumber.DoClick = function()
		local number = entry:GetValue()
		if number == "" then return end
		UI.selectedNumber = number
		panel:Close()
	end

	local public = vgui.Create("DCheckBoxLabel", panel)
	public:SetPos(12, 270)
...
local function BuildPhonePanel(phone)
	...
	registryLabel:SetText("CONTACTS — MAP AND PLAYERS")
	...
	local contactTabs = vgui.Create("DPropertySheet", frame)
	contactTabs:SetPos(16, 64)
	contactTabs:SetSize(430, 302)
	local contactsPanel = vgui.Create("DPanel", contactTabs)
	contactTabs:AddSheet("CONTACTS", contactsPanel)
	registry:SetParent(contactsPanel)
	registry:SetPos(0, 0)
	registry:SetSize(430, 280)
	registryLabel:SetVisible(false)
	...
	chatLabel:SetText("TEXT MESSAGES")
	...
	messageEntry:SetPlaceholderText("Text the selected contact")
	...
	local function SendText()
		...
		net.Start("HG_Phone_Text")
			net.WriteEntity(phone)
			net.WriteString(UI.selectedNumber or "")
	...
	frame.Think = function(self)
		...
		sendButton:SetEnabled(UI.selectedNumber ~= nil)
		messageEntry:SetEnabled(UI.selectedNumber ~= nil)
	end
