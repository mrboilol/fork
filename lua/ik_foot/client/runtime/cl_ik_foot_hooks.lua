...
hook.Add("PostPlayerDraw", "IKFoot_PostPlayerDraw", function(ply)
	...
	if ply == lp and isfunction(hg and hg.ShouldTPIK) and hg.ShouldTPIK(ply) then
		...
		if IsValid(wpn) and (wpn.lhandik or wpn.rhandik) then return end
	end
	...
	-- skip if organism is in a critical/ragdoll state to avoid animation conflicts
	if ply.organism and (ply.organism.otrub or (ply.organism.brain or 0) > 0.1) then
		RT.Apply.ResetPlayer(ply)
		return
	end
