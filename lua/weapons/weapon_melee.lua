...
function SWEP:PlayEffects(trace, attacktype)
    ...
    elseif not self.AttackHitPlayed then
		...
		if (tonumber(self.weight) or 0) >= 1.5 and self.DamageType ~= DMG_SLASH and trace.MatType ~= MAT_GLASS and not self:IsSecondaryAttackType(attacktype) then
...
function SWEP:Feint()
	...
	if owner.organism then
		...
		self:ConsumeMeleeStamina(owner, staminaCost * (self.StaminaCostMul or 0.5) * 0.5 * math.Clamp(vellen / 200, 1, 1.25))
    end
