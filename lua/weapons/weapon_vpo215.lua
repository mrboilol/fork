...
function SWEP:Reload(time)
    ...
    if SERVER then
        ...
        if isEmpty then
            ...
            timer.Simple(2.00, function() if IsValid(wep) then wep:EmitSound(path .. "rem700_mag_in.ogg") end end)
            timer.Simple(2.60, function() if IsValid(wep) then wep:EmitSound(path .. "rem700_bolt_1.ogg") end end)
            timer.Simple(3.40, function() if IsValid(wep) then wep:EmitSound(path .. "rem700_bolt_4.ogg") end end)
        else
            -- Тактическая: Out 0.5, In 2.3
            timer.Simple(0.50, function() if IsValid(wep) then wep:EmitSound(path .. "rem700_mag_out.ogg") end end)
            timer.Simple(2.30, function() if IsValid(wep) then wep:EmitSound(path .. "rem700_mag_in.ogg") end end)
        end
    end
end
