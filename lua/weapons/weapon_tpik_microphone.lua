...
SWEP.WorldModel = "models/wwe/microphone.mdl"
SWEP.WorldModelReal = "models/weapons/combatknife/tactical_knife_iw7_vm.mdl"
SWEP.WorldModelExchange = "models/wwe/microphone.mdl"
SWEP.DontChangeDropped = true

if CLIENT then
	SWEP.WepSelectIcon = Material("vgui/new_icons/melee/microphone_new")
	SWEP.IconOverride = "vgui/new_icons/melee/microphone_new"
	SWEP.BounceWeaponIcon = false
end

SWEP.HoldPos = Vector(0,10,2)
SWEP.HoldAng = Angle(40,0,0)
SWEP.weaponPos = Vector(-2.5,0.3,-0.7)
...
function SWEP:CustomBlockAnim(addPosLerp, addAngLerp)
    addPosLerp.z = addPosLerp.z + (self:GetBlocking() and 0 or 0)
    addPosLerp.x = addPosLerp.x + (self:GetBlocking() and 27 or 0)
    addPosLerp.y = addPosLerp.y + (self:GetBlocking() and 0 or 0)
    addAngLerp.r = addAngLerp.r + (self:GetBlocking() and 0 or 0)
    addAngLerp.y = addAngLerp.y + (self:GetBlocking() and 0 or 0)
    addAngLerp.p = addAngLerp.p + (self:GetBlocking() and 80 or 0)
